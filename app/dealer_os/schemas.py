...
class SessionCreate(BaseModel):
    ...
    @classmethod
    def _reject_client_join_url(cls, value: object) -> object:
        if isinstance(value, dict) and value.get("join_url") not in (None, ""):
            raise ValueError(
                "Join URLs are generated by the configured calendar provider."
            )
        return value
...
class SharedBookingHostRead(BaseModel):
    id: UUID
    name: str
    email: str


class SharedBookingCalendarConnectionRead(BaseModel):
    status: Literal["connected", "disconnected", "error"]
    email: str | None = None
    last_error: str | None = None


class SharedBookingSettingsRead(BaseModel):
    host: SharedBookingHostRead
    settings: UserBookingSettingsRead
    can_edit: bool
    public_url: str | None = None
    calendar_connection: SharedBookingCalendarConnectionRead
